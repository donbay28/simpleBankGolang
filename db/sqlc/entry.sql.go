// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.18.0
// source: entry.sql

package db

import (
	"context"
	"database/sql"
)

const createEntries = `-- name: CreateEntries :one
INSERT INTO entries (
  account_id, 
  amount
) VALUES (
  $1, $2
)  RETURNING id_entries, account_id, amount, created_at
`

type CreateEntriesParams struct {
	AccountID sql.NullInt64 `json:"account_id"`
	Amount    int64         `json:"amount"`
}

func (q *Queries) CreateEntries(ctx context.Context, arg CreateEntriesParams) (Entry, error) {
	row := q.db.QueryRowContext(ctx, createEntries, arg.AccountID, arg.Amount)
	var i Entry
	err := row.Scan(
		&i.IDEntries,
		&i.AccountID,
		&i.Amount,
		&i.CreatedAt,
	)
	return i, err
}

const deleteEntries = `-- name: DeleteEntries :exec
DELETE FROM entries WHERE id_entries = $1
`

func (q *Queries) DeleteEntries(ctx context.Context, idEntries int64) error {
	_, err := q.db.ExecContext(ctx, deleteEntries, idEntries)
	return err
}

const getEntries = `-- name: GetEntries :one
SELECT id_entries, account_id, amount, created_at FROM entries
WHERE id_entries = $1 LIMIT 1
`

func (q *Queries) GetEntries(ctx context.Context, idEntries int64) (Entry, error) {
	row := q.db.QueryRowContext(ctx, getEntries, idEntries)
	var i Entry
	err := row.Scan(
		&i.IDEntries,
		&i.AccountID,
		&i.Amount,
		&i.CreatedAt,
	)
	return i, err
}

const listEntries = `-- name: ListEntries :many
SELECT id_entries, account_id, amount, created_at FROM entries
ORDER BY id_entries LIMIT $1 OFFSET $2
`

type ListEntriesParams struct {
	Limit  int32 `json:"limit"`
	Offset int32 `json:"offset"`
}

func (q *Queries) ListEntries(ctx context.Context, arg ListEntriesParams) ([]Entry, error) {
	rows, err := q.db.QueryContext(ctx, listEntries, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []Entry
	for rows.Next() {
		var i Entry
		if err := rows.Scan(
			&i.IDEntries,
			&i.AccountID,
			&i.Amount,
			&i.CreatedAt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const updateEntries = `-- name: UpdateEntries :one
UPDATE entries SET amount = $2
WHERE id_entries = $1
RETURNING id_entries, account_id, amount, created_at
`

type UpdateEntriesParams struct {
	IDEntries int64 `json:"id_entries"`
	Amount    int64 `json:"amount"`
}

func (q *Queries) UpdateEntries(ctx context.Context, arg UpdateEntriesParams) (Entry, error) {
	row := q.db.QueryRowContext(ctx, updateEntries, arg.IDEntries, arg.Amount)
	var i Entry
	err := row.Scan(
		&i.IDEntries,
		&i.AccountID,
		&i.Amount,
		&i.CreatedAt,
	)
	return i, err
}
